...
function duckStart() {
  ...
  dinoImg.style.height = "40px";
}
function duckEnd() {
  if (!gameStarted || !isDucking || gameOver) return;
  isDucking = false;
  dinoImg.src = "img/dino.png";
  dinoImg.style.height = "60px";
}
...
/***** 장애물 (선인장 + 익룡) *****/
const cactusOptions = [
  { src: "img/long cactus.png", w: 35, h: 90 },
  { src: "img/middle cactus.png", w: 30, h: 70 },
  { src: "img/short cactus.png", w: 25, h: 50 },
  { src: "img/pterosaur.png", w: 80, h: 60 },
];
...
function spawnLoop() {
  ...
  // 📱 모바일은 1초(1000ms) 더 느리게
  const baseDelay = isMobile ? 2000 : 1000; // 기본 딜레이
  const randomRange = isMobile ? 1500 : 1500; // 랜덤 범위 동일
...
function adjustDinoSize() {
  ...
  dinoImg.style.width = 30 * scale + "px";
  dinoImg.style.height = 30 * scale + "px";
}
